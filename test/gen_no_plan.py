...
class Generator(object):
    ...
    def translate_one(self, input_nodes, adj_matrix, node_lens, scene):
        dec_ids, attn_weights = self.model.predict(
            input_nodes, adj_matrix, node_lens, scene, self.device, self.model_opt.max_token_seq_len+1)
        return dec_ids
    ...
    def translate_with_beam(self, input_equ_nodes, adj_equ_matrix, equ_node_lens, input_sns_nodes, adj_sns_matrix, sns_node_lens, scene):
        attns, res = beam_search_graph_gate_vae(self.model, input_equ_nodes, adj_equ_matrix, equ_node_lens, input_sns_nodes,
                                                adj_sns_matrix, sns_node_lens, scene, self.device, self.model_opt.max_token_seq_len+1, self.beam_width, self.output_beam)
        return res
...
for batch in test_loader:
    equ_nodes, sns_nodes, equ_node_lens, sns_node_lens, equ_adj_matrixs, sns_adj_matrixs, tgt_seq, scene = map(
...
def end2end_beam(input_equ_nodes, adj_equ_matrix, equ_node_lens, input_sns_nodes, adj_sns_matrix, sns_node_lens, scene, lex, idx2word, generator):
    output = generator.translate_with_beam(
        input_equ_nodes, adj_equ_matrix, equ_node_lens, input_sns_nodes, adj_sns_matrix, sns_node_lens, scene)
    all_text = []
...
def beam_search_graph_gate_vae(model, input_equ_nodes, adj_equ_matrix, equ_node_lens, input_sns_nodes, adj_sns_matrix, sns_node_lens, scene, device, max_tgt_len, beam_width=3, output_num=3):
    ...
    with torch.no_grad():
        ...
        equ_encoder_outputs, equ_encoder_hidden = model.encoder_equ(
        ...
        sns_encoder_outputs, sns_encoder_hidden = model.encoder_sns(
        ...
        prior_mu, prior_logvar = model.q_mu_prior(
            prior_embedding), model.q_logvar_prior(prior_embedding)
        ...
        node = GateNode(torch.cat(
            [cond_embedding, latent_sample], dim=1), None, decoder_input, None, 0, 1)
        q = Queue()
        q.put(node)
        end_nodes = []
        while not q.empty():
...
#                 print("d_dec shape", d_dec.shape)
                ...
                log_prob, hidden, attn = model.decoder(
                    prev_y, hidden, equ_encoder_outputs, sns_encoder_outputs, latent_sample, input_equ_node_mask=None, input_sns_node_mask=None)
                ...
                for k in range(beam_width):
                    ...
                    child = GateNode(hidden.squeeze(
                        1), node, index, attn, node.log_prob + log_p, node.length+1)
        ...
        for one_node in node:
            ...
            while one_node.previous_node != None:
                ...
                one_attns.append(one_node.attn.squeeze(
                    0).cpu().numpy().tolist())
